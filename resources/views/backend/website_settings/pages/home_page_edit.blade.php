...
<div class="row">
	<div class="col-xl-10 mx-auto">
		...
		<div class="card">
			<div class="card-header">
				<h6 class="mb-0">{{ translate('Sidebar Categories') }}</h6>
			</div>
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="form-group row">
						<label class="col-md-2 col-from-label">{{translate('Sidebar Categories')}}</label>
						<div class="col-md-10">
		...
		<div class="card">
			<div class="card-header">
				<h6 class="mb-0">{{ translate('Categoies Products') }}</h6>
			</div>
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="form-group row">
						...
						<div class="col-md-10">
							...
							<select name="categories_products[]" class="form-control aiz-selectpicker" multiple data-max-options="10" data-live-search="true" data-selected={{ get_setting('categories_products') }}>
								@foreach (\App\Category::where('parent_id', 0)->with('childrenCategories')->get() as $category)
									<option value="{{ $category->id }}">{{ $category->getTranslation('name') }}</option>
									@foreach ($category->childrenCategories as $childCategory)
										@include('categories.child_category', ['child_category' => $childCategory])
		...
		<div class="card">
			<div class="card-header">
				<h6 class="mb-0">{{ translate('Facebook Videos') }}</h6>
			</div>
			<div class="card-body">
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					@csrf
					<div class="form-group">
						<label>{{ translate('Facebook Video Post URL') }}</label>
						<div class="facebook-post-video">
							<input type="hidden" name="types[]" value="facebook_video_urls">
							@if (get_setting('facebook_video_urls') != null)
								@foreach (json_decode(get_setting('facebook_video_urls'), true) as $key => $value)
									<div class="row gutters-5">
										<div class="col-md">
											<div class="form-group">
												<input type="hidden" name="types[]" value="facebook_video_urls">
												<input type="text" class="form-control" placeholder="http://" name="facebook_video_urls[]" value="{{ json_decode(get_setting('facebook_video_urls'), true)[$key] }}">
											</div>
										</div>
										<div class="col-md-auto">
											<div class="form-group">
						...
						<button
							...
							<div class="row gutters-5">
								<div class="col-md">
									<div class="form-group">
										<input type="hidden" name="types[]" value="facebook_video_urls">
										<input type="text" class="form-control" placeholder="http://" name="facebook_video_urls[]">
									</div>
								</div>
								<div class="col-md-auto">
									<div class="form-group">
							...
							data-target=".facebook-post-video">
							{{ translate('Add New') }}
						</button>
					</div>
					<div class="text-right">
