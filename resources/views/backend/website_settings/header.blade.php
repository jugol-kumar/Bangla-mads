				...
				<form action="{{ route('business_settings.update') }}" method="POST" enctype="multipart/form-data">
					...
					<div class="border-top pt-3">
						...
						<div class="header-nav-menu">
							<input type="hidden" name="types[]" value="main_header_menu_labels">
							<input type="hidden" name="types[]" value="main_header_menu_links">
							@if (get_setting('main_header_menu_labels') != null)
								@foreach (json_decode( get_setting('main_header_menu_labels'), true) as $key => $value)
									<div class="row gutters-5">
										<div class="col-4">
											<div class="form-group">
												<input type="text" class="form-control" placeholder="Label" name="main_header_menu_labels[]" value="{{ $value }}">
											</div>
										</div>
										<div class="col">
											<div class="form-group">
												<input type="text" class="form-control" placeholder="{{ translate('Link with') }} http:// {{ translate('or') }} https://" name="main_header_menu_links[]" value="{{ json_decode(App\BusinessSetting::where('type', 'main_header_menu_links')->first()->value, true)[$key] }}">
											</div>
										</div>
										<div class="col-auto">
						...
						<button
							...
							data-content='<div class="row gutters-5">
								<div class="col-4">
									<div class="form-group">
										<input type="text" class="form-control" placeholder="Label" name="main_header_menu_labels[]">
									</div>
								</div>
								<div class="col">
									<div class="form-group">
										<input type="text" class="form-control" placeholder="{{ translate('Link with') }} http:// {{ translate('or') }} https://" name="main_header_menu_links[]">
									</div>
								</div>
								<div class="col-auto">
									<button type="button" class="mt-1 btn btn-icon btn-circle btn-sm btn-soft-danger" data-toggle="remove-parent" data-parent=".row">
					...
					<div class="mt-3">
						<button type="submit" class="btn btn-secondary">{{ translate('Update') }}</button>
					</div>
				</form>
			</div>
		</div>
