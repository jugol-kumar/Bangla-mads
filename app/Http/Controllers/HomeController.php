...
//                ->orWhere('id', '<', $request->request_product_id)->orWhere('category_id', $category_id)
                ...
                $load_more = filter_products($products)->latest()->take(12)->get();

                if ($load_more->count() > 0){
                    return view('frontend.partials.load_more_product_list', compact('load_more'));
                }else{
                    return false;
                }
            }else{
        ...
        $nonPagisateProducts = filter_products($products)->take(12)->get();
        if ($query != null){
            if ($request->ajax()){
                if($query != null){
                    $searchController = new SearchController;
                ...
                $load_more = filter_products($products)->latest()->take(12)->get();
                return view('frontend.partials.search_table_product_listing', compact('load_more'));
            }else{
                $products = filter_products($products)->paginate(12)->appends(request()->query());
                return view('frontend.search_product_listing', compact( 'products','nonPagisateProducts', 'query', 'category_id', 'brand_id', 'sort_by', 'seller_id','min_price', 'max_price', 'attributes', 'selected_attributes', 'all_colors', 'selected_color','city_id'));
            }
        }
    ...
    public function get_single_flash_deals() {
        $today = strtotime(date('Y-m-d H:i:s'));

        $data['all_flash_deals'] = FlashDeal::inRandomOrder()->where('status', 1)
                ->where('start_date', "<=", $today)
                ->where('end_date', ">", $today)
                ->orderBy('created_at', 'desc')
                ->first();
    ...
    {   $invest = new Invest;
        ...
        $invest->phone = $request->phone;
        $invest->age = $request->age;
        $invest->present = $request->present;
        $invest->permanent = $request->permanent;
        $invest->income_source = $request->income_source;
        $invest->educational_qualification = $request->educational_qualification;
        $invest->alem_names = $request->alem_names;

        $imageName = time().'.'.$request->photo->getClientOriginalExtension();
        $request->photo->move(public_path('uploads'), $imageName);

        $invest->photo = $imageName;
        $invest->save();
    ...
    public function upload_prescriptin_image(Request $request){


        $imageName = [];
        $getImage = $request->file('prescription_image');
        if(isset($getImage)){
            foreach ($request->file('prescription_image') as $key=> $image){
                $ext = $image->getClientOriginalExtension();
                $fullName =  time().random_int(1, 1000).'.'.$ext;
                $imageName[] = $fullName;
                $image->storeAs('uploads/prescriptions', $fullName);
            }
        }
...
//            try {
                UserPrescription::create([
                    ...
                    'user_message' => $request->details_message,
                    'prescription_image' => json_encode($imageName)
                ]);
                flash("Prescription Upload Successfully Done. Please Order Now...")->success();
                return back();
//            }catch (\Exception $e){
