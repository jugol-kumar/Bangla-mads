...
if (! function_exists('sendSMS')) {
    function sendSMS($to, $from, $text)
    {
        ...
        elseif (OtpConfiguration::where('type', 'BulkSmsBD')->first()->value == 1) {
            ...
            $api_username = env("BulkSmsBD_USERNAME");
            $api_password = env("BulkSmsBD_PASSWORD");
...
if (! function_exists('my_asset')) {
    ...
    function my_asset($path, $secure = null)
    {
        if(env('FILESYSTEM_DRIVER') == 's3'){
            ...
            return env('APP_URL') == 'http://127.0.0.1:8000' ? app('url')->asset('/'.$path, $secure) : app('url')->asset('public/'.$path, $secure);
        }
    }
}
...
if (!function_exists('getFileBaseURL')) {
    function getFileBaseURL()
    {
        if(env('FILESYSTEM_DRIVER') == 's3'){
            ...
            return env('APP_URL') == 'http://127.0.0.1:8000' ? getBaseURL() : getBaseURL().'public/';
        }
    }
}
